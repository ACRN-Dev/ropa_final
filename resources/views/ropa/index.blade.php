...
@section('title', 'User | ROPA Management')
...
<div class="container mx-auto px-4 py-6 max-w-7xl">
    ...
    <div class="flex gap-2 mb-6 border-b border-gray-200">
        <button type="button" 
                ...
                id="create-tab" 
                class="tab-button active px-4 py-2 text-sm font-semibold text-orange-600 border-b-2 border-orange-600 hover:text-orange-700 transition-colors">
        ...
        <button type="button" 
                ...
                id="all-tab" 
                class="tab-button px-4 py-2 text-sm font-semibold text-gray-600 hover:text-gray-800 transition-colors">
            <svg class="w-4 h-4 inline-block mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>
            All ROPAs ({{ isset($ropas) ? $ropas->total() ?? $ropas->count() : 0 }})
        </button>
    </div>
    ...
    <div id="all-content" class="tab-content hidden">
        @if(isset($ropas) && $ropas->count() > 0)
            <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
                    ...
                    <div class="flex flex-col lg:flex-row gap-3 justify-between items-start lg:items-center">
                        ...
                        <div class="flex-1 max-w-md w-full">
                            <div class="relative">
                                <input type="text" 
                                       id="searchInput"
                                       placeholder="Search organisations, departments..."
                        ...
                        <div class="flex items-center gap-2 w-full lg:w-auto flex-wrap">
                            ...
                            <button type="button" 
                                    id="bulkDeleteBtn"
                                    onclick="bulkDelete()"
                                    class="hidden items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all text-sm font-semibold shadow-sm hover:shadow-md">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            {{-- Record Count --}}
                            <div class="flex items-center gap-2 text-xs text-gray-600 bg-white px-3 py-2 rounded-lg border border-gray-200 shadow-sm">
                                <svg class="w-4 h-4 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                </svg>
                                <span class="font-bold text-gray-900">{{ $ropas->total() ?? $ropas->count() }}</span>
                                <span>Records</span>
                            </div>
                            
                            {{-- Export Button --}}
                            <button onclick="window.print()" 
                                    class="inline-flex items-center gap-2 px-4 py-2 bg-gray-700 text-white rounded-lg hover:bg-gray-800 transition-all text-sm font-semibold shadow-sm hover:shadow-md">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                                </svg>
                ...
                {{-- Bulk Delete Form --}}
                <form id="bulkDeleteForm" action="{{ route('ropa.bulk-delete') }}" method="POST" style="display: none;">
                    @csrf
                    @method('DELETE')
                    ...
                    <table class="w-full text-sm" id="ropaTable">
                            ...
                            <tr class="bg-gradient-to-r from-orange-600 via-orange-500 to-orange-600 text-white">
                                ...
                                <th class="px-4 py-3 text-left w-12">
                                    <input type="checkbox" 
                                           ...
                                           class="w-4 h-4 rounded border-2 border-white text-orange-600 focus:ring-2 focus:ring-white cursor-pointer">
                                </th>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider min-w-[200px]">
                                    <div class="flex items-center gap-1">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($ropas as $ropa)
                                <tr class="hover:bg-orange-50 transition-all duration-150" data-ropa-id="{{ $ropa->id }}">
                                    {{-- Checkbox --}}
                                    <td class="px-4 py-3">
                                        <input type="checkbox" 
                                               class="row-checkbox w-4 h-4 rounded border-gray-300 text-orange-600 focus:ring-2 focus:ring-orange-500 cursor-pointer"
                                               value="{{ $ropa->id }}"
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10 bg-gradient-to-br from-orange-400 to-orange-600 rounded-lg flex items-center justify-center shadow-sm">
                                            ...
                                            <div class="ml-3">
                                                ...
                                                <div class="text-xs text-gray-500">ID: #{{ $ropa->id }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-3">
                                        @php
                                            ...
                                            if (is_string($processes)) {
                                                $decoded = json_decode($processes, true);
                                                if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                                                    $processes = $decoded;
                                                }
                                            }
                                            if (is_array($processes)) {
                                                $processText = implode(', ', array_filter($processes));
                                            } else {
                                                $processText = $processes ?? 'N/A';
                                            }
                                        @endphp
                                        <div class="text-sm text-gray-900 max-w-xs">
                                            <div class="line-clamp-2 leading-relaxed" title="{{ $processText }}">
                                                {{ $processText }}
                                            </div>
                                            @if(strlen($processText) > 80)
                                                <a href="{{ route('ropa.show', $ropa->id) }}" class="text-xs text-orange-600 hover:text-orange-800 font-medium mt-1 inline-flex items-center gap-1">
                                                    View more
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-8 w-8 bg-gradient-to-br from-gray-300 to-gray-400 rounded-full flex items-center justify-center shadow-sm">
                                            ...
                                            <div class="ml-2">
                                                ...
                                                @if($ropa->user && $ropa->user->email)
                                                    <div class="text-xs text-gray-500">
                                                        {{ Str::limit($ropa->user->email, 20) }}
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <div class="flex items-center justify-center gap-1">
                                            ...
                                            <a href="{{ route('ropa.edit', $ropa->id) }}"
                                               class="p-2 text-orange-600 hover:text-orange-800 hover:bg-orange-50 rounded-lg transition-all duration-150"
                                               title="Edit Record">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <form action="{{ route('ropa.destroy', $ropa->id) }}"
                                                  ...
                                                  onsubmit="return confirm('⚠️ Are you sure you want to permanently delete this ROPA record?\n\nThis action cannot be undone.');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                    ...
                    <div class="px-4 py-3 border-t border-gray-200 bg-gray-50">
                        <div class="flex flex-col sm:flex-row items-center justify-between gap-3">
                            <div class="text-sm text-gray-600">
                                Showing <span class="font-bold text-gray-900">{{ $ropas->firstItem() }}</span> to 
                                <span class="font-bold text-gray-900">{{ $ropas->lastItem() }}</span> of 
                                <span class="font-bold text-gray-900">{{ $ropas->total() }}</span> results
                            </div>
                            <div>
                                {{ $ropas->links() }}
                            </div>
        ...
        @else
            ...
            <div class="bg-white rounded-xl shadow-lg p-12 text-center border border-gray-200">
                <div class="max-w-lg mx-auto">
                    ...
                    <h3 class="text-2xl font-bold text-gray-900 mb-3">No ROPA Records Yet</h3>
                    <p class="text-gray-600 mb-8 leading-relaxed">
                        Start building your Register of Processing Activities by creating your first record. 
                        Track and manage your data processing activities effectively.
                    </p>
                    <button type="button" 
                            onclick="switchTab('create')"
...
function bulkDelete() {
    ...
    const confirmMessage = `⚠️ Bulk Delete Confirmation\n\nYou are about to permanently delete ${ids.length} ROPA record(s).\n\nThis action cannot be undone. Are you sure you want to continue?`;
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                // Search in relevant columns (skip checkbox and actions)
            ...
            if (visibleCount === 0 && filter !== '') {
                if (!noResultsRow) {
                    ...
                    newRow.innerHTML = '<td colspan="7" class="px-6 py-12 text-center text-gray-500"><div class="flex flex-col items-center gap-3"><svg class="w-12 h-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><p class="text-lg font-bold text-gray-700">No results found</p><p class="text-sm text-gray-500">Try adjusting your search terms</p></div></td>';
                }
...
<style>
@media print {
    .no-print, button, .tab-button, input[type="checkbox"] { 
