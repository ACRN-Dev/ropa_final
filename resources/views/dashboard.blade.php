...
function updateRopaSelection() {
    ...
    document.getElementById('select-all-ropa').checked = count === allCheckboxes.length && count > 0;
}
...
function bulkShareRopa() {
    ...
    if (ids.length === 0) { alert('Please select at least one ROPA record'); return; }
...
function toggleAllRisks(checkbox) {
    document.querySelectorAll('.risk-checkbox').forEach(cb => cb.checked = checkbox.checked);
...
function updateRisksSelection() {
    ...
    document.getElementById('risks-selected-count').textContent = count;

    count > 0 ? bulkActions.classList.remove('hidden') : bulkActions.classList.add('hidden');

    const allCheckboxes = document.querySelectorAll('.risk-checkbox');
    document.getElementById('select-all-risks').checked = count === allCheckboxes.length && count > 0;
}
...
function bulkUpdateRiskStatus() {
    ...
    if (ids.length === 0) { alert('Please select at least one risk'); return; }
...
function bulkDeleteRisks() {
    ...
    if (confirm(`Are you sure you want to delete ${ids.length} risk(s)?`)) {
        ...
        // window.location.href = `/risk-register/bulk-delete?ids=${ids.join(',')}`;
    }
}
...
function openShareModal(ropaId) {
    ...
    document.getElementById('shareForm').action = "/ropa/" + ropaId + "/send-email";
}
